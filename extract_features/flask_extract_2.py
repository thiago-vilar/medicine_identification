...
class ExtractFeatures:
    ...
    def medicine_measures(self, removed_background, largest_contour):
        ...
        measures = []
        measured_img = removed_background.copy()
        ...
        # Salva a imagem medida localmente
        # cv2.imwrite('measured_medicine.png', measured_img)
    ...
    def process_images(self):
        ...
        if corners_image is not None :
            ...
            self.display_image(cropped_image, 'Cropped')   
            ...
            # filter_pdi = self.filter_pdi(cropped_image)
            # self.display_image(filter_pdi, 'Laplacian Gray Balance')

            remove_background = self.remove_background(cropped_image)
            self.display_image(remove_background, 'Removed Background')   
            
            histograms, color_stats = self.calculate_histograms(remove_background)
            self.display_histogram(histograms, color_stats)
...
def main():
    stag_id = 0
    med_type = "Flask"
    image_path = "./extract_features/img_0_010.jpg"
    # neg_image_path = "./thiago_fotos_black_t1/img_7_009.jpg"
